...
void blink() {
	...
	if (timer == 0x7A12) {  // if the clock has come to the time set, turn on the segment. (0x7A12)
		LCDDR0 |= 0x6;
	}
	else if(timer == 0xF424) {  // if the clock has come to two times the time set, turn off segment. (0xF424)
		LCDDR0 |= 0x0;
		lastTime = timer;
	}
}
